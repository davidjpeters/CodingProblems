/*
C++ translation for "How many stairs will Suzuki climb in 20 years?"" - User8476848
some code commented out to avoid errors for missing igloo framework
*/

// Solution
#include <numeric>

int stairsin20(const std::vector<std::vector<int>>& stairs) {
    int one_year_total = 0;
  
    for (const auto& day : stairs) {
        one_year_total += std::accumulate(day.begin(), day.end(), 0);
    }
...
void runRandomTest() {
    ...
    std::vector<std::vector<int>> q(7);
    for (auto& day : q) {
        day.resize(52);
        std::generate(day.begin(), day.end(), []() { return rand() % 10000 + 1; });
    }
    
    // calculate expected value
    int expected = 20 * std::accumulate(q.begin(), q.end(), 0LL, 
        [](int sum, const std::vector<int>& day) {
            return sum + std::accumulate(day.begin(), day.end(), 0);
        });

    //Assert::That(stairsin20(q), Equals(expected), ExtraMessage("Random test failed"));
}

/*
#include <igloo/igloo_alt.h>
using namespace igloo;
// Test suite (fixed and random)
Describe(tests)
{
    It(sampleTest)
    {
        Assert::That(stairsin20({{6737, 7244, 5776, 9826, 7057, 9247, 5842, 5484, 6543, 5153, 6832, 8274,
             ...
             {5448, 8041, 6573, 8104, 6208, 5912, 7927, 8909, 7000, 5059, 6412, 6354, 8943,
              ...
              7150, 6567, 8524, 7613, 7809, 5562, 7799, 7179, 5184, 7960, 9455, 5633, 9085}}), Equals(54636040));
    }
    
    It(randomTest)
    {
        for (int i = 0; i < 40; ++i) { // run 3 random tests
            runRandomTest();
        }
    }
};

// Sample test case (user sees this)
Describe(tests)
{
    It(sampleTest)
    {
